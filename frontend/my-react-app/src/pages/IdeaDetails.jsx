...
export default function IdeaDetails() {
  ...
  useEffect(() => {
    const fetchIdea = async () => {
      try {
        ...
        console.error(err);
        setError("Could not fetch idea details. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBack = () => setLocation("/"); // go back to sidebar or list page
  ...
  // Use generatedData from backend
  const generated = idea.generatedData || {};
    ...
    <span className="relative text-base font-extrabold text-gray-900 tracking-wide">
      ...
      >
        {sections.map((section, idx) => {
          const content = generated[section.key];
          ...
          return (
            ...
            >
              <div className={`text-lg font-bold mb-3 ${section.textColor}`}>{section.title}</div>
